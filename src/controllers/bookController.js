...
const updateBook = async function (req, res) {
  try {
    ...
    //user can update title,excerpt,releasedDate,ISBN only
    ...
    if (checkDuplicateValue) {
      return res.status(409).send({
        status: false,
        message: "you can not update duplicate value , TITLE or ISBN",
      });
    }
    ...
    await bookModel.findByIdAndUpdate({ _id: getBookId }, getBody);

    let findUpdatedBook = await bookModel.findOne(
      { _id: getBookId },
      { deletedAt: 0, __v: 0 }
    );
    return res
...
const deleteBook = async function (req, res) {
  try {
    ...
    //find book from that id whether is it exist or not
